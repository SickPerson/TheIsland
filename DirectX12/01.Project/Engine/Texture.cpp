...
void CTexture::Load( const vector<wstring>& vecFullPath )
{
	//for ( size_t i = 0; i < vecFullPath.size(); ++i )
	//{
	//	// 확장자명 얻기
	//	wchar_t szExt[50] = L"";
	//	_wsplitpath_s( vecFullPath[i].c_str(), nullptr, 0, nullptr, 0, nullptr, 0, szExt, 50 );
	...
	//		if ( FAILED( LoadFromDDSFile( vecFullPath[i].c_str(), DDS_FLAGS_NONE, nullptr, *pImage )) )
	//		{
	//			assert( nullptr );
	//		}
	//	}
	//	else if ( L".tga" == strExt || L".TGA" == strExt )
	//	{
	//		if ( FAILED( LoadFromTGAFile( vecFullPath[i].c_str(), nullptr, *pImage ) ) )
	//		{
	//			assert( nullptr );
	//		}
	//	}
	//	else // png, jpg, jpeg, bmp (WIC)
	//	{
	//		if ( FAILED( LoadFromWICFile( vecFullPath[i].c_str(), WIC_FLAGS_FORCE_RGB, nullptr, *pImage ) ) )
	//		{
	//			assert( nullptr );
	//		}
	//	}
	...
	//vector< ComPtr<ID3D12Resource>> vecTex;

	//for ( size_t i = 0; i < m_vecImage.size(); ++i )
	//{

	//	HRESULT hr = CreateTexture( DEVICE.Get(), m_vecImage[i]->GetMetadata(), &pTex );
	//	if ( FAILED( hr ) )
	//		assert( nullptr );
	...
	//	// upload is implemented by application developer. Here's one solution using <d3dx12.h>
	//	const UINT64 uploadBufferSize = GetRequiredIntermediateSize( pTex.Get(), 0, static_cast< unsigned int >( vecSubresources.size() ) );
	...
	//D3D12_SHADER_RESOURCE_VIEW_DESC srvDesc = {};
	//srvDesc.Shader4ComponentMapping = D3D12_DEFAULT_SHADER_4_COMPONENT_MAPPING;
	//srvDesc.Format = m_tDesc.Format;
	//srvDesc.ViewDimension = D3D12_SRV_DIMENSION_TEXTURE2DARRAY;
	//srvDesc.Texture2DArray.MipLevels = m_tDesc.MipLevels;
	//srvDesc.Texture2DArray.ArraySize = vecTex.size();
	//DEVICE->CreateShaderResourceView( m_pTex2D.Get(), &srvDesc, m_pSRV->GetCPUDescriptorHandleForHeapStart() );
