...
{
	Vec3 vScale = m_tLandScapeInfo.pTransform->GetLocalScale();
	...
	Vec3 vLandScapePos = m_tLandScapeInfo.pTransform->GetLocalPos();
	...
	float fX, fY;
	fX = ( vPos.x - ( *m_tLandScapeInfo.pvecPos)[iIdx].x ) / fWidth;
	fY = ( ( *m_tLandScapeInfo.pvecPos )[iIdx].z - vPos.z ) / fHeight;
