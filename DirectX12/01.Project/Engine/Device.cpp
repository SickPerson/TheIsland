...
void CDevice::SetBufferToSRVRegister_CS( CStructuredBuffer * pBuffer, TEXTURE_REGISTER eRegister )
{
	UINT iDestRange = 1;
	UINT iSrcRange = 1;
	...
	{
		CMDLIST_CS->ResourceBarrier( 1, &CD3DX12_RESOURCE_BARRIER::Transition( pBuffer->GetBuffer().Get()
			, D3D12_RESOURCE_STATE_UNORDERED_ACCESS, D3D12_RESOURCE_STATE_COMMON ) );
...
void CDevice::SetBufferToUAVRegister_CS( CStructuredBuffer * pBuffer, UAV_REGISTER eRegister )
{
	UINT iDestRange = 1;
	UINT iSrcRange = 1;
	...
	// 리소스 상태 변경
	if ( pBuffer->GetResState() == D3D12_RESOURCE_STATE_COMMON )
	{
		CMDLIST_CS->ResourceBarrier( 1, &CD3DX12_RESOURCE_BARRIER::Transition( pBuffer->GetBuffer().Get()
			, D3D12_RESOURCE_STATE_COMMON, D3D12_RESOURCE_STATE_UNORDERED_ACCESS ) );

		pBuffer->SetResState( D3D12_RESOURCE_STATE_UNORDERED_ACCESS );
	}
}
...
void CDevice::SetBufferToRegister( CStructuredBuffer * pBuffer, TEXTURE_REGISTER eRegister )
{
	UINT iDestRange = 1;
	UINT iSrcRange = 1;
