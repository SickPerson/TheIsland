...
void CResourceManager::CreateDefaultMesh()
{
	...
	AddResource( L"ColRectMesh", pMesh );
	...
	AddResource( L"ColCircleMesh", pMesh );
	...
	AddResource<CMesh>( pMesh->GetName(), pMesh );
...
void CResourceManager::CreateDefaultShader()
{
	...
	AddResource( L"TexShader", pShader );
	...
	pShader->Create();
	AddResource( L"Collider2DShader", pShader );
	...
	pShader->Create();
	AddResource( L"Std2DShader", pShader );
	...
	pShader->AddShaderParam( tShaderParam{ L"Output Texture", SHADER_PARAM::TEX_0 } );
	pShader->Create();
	AddResource( L"2DShadowShader", pShader );
	...
	pShader->Create();
	AddResource( L"Std3DShader", pShader );
	...
	pShader->Create();
	AddResource( L"SkyboxShader", pShader );
}
...
void CResourceManager::CreateDefaultMaterial()
{
	...
	AddResource( L"TestMtrl", pMtrl );

	pMtrl = new CMaterial;
	pMtrl->DisableFileSave();
	pMtrl->SetShader( FindRes<CShader>( L"TexShader" ) );
	AddResource( L"TexMtrl", pMtrl );
	...
	AddResource( L"Collider2DMtrl_0", pMtrl );
...
const map<wstring, CResource*>& CResourceManager::GetResource( RES_TYPE eType )
{
	...
	return m_mapResource[( UINT )eType];
}

template<typename T>
inline void CResourceManager::AddResource( const wstring & strKey, Ptr<T> pRes )
{
	...
	m_mapResource[( UINT )eType].insert( make_pair( strKey, *ppRes ) );
	pRes->SetName( strKey );
}

template<typename T>
void CResourceManager::AddCloneResource( Ptr<T> pCloneRes )
{
	static const type_info& mesh = typeid( CMesh );
	static const type_info& shader = typeid( CShader );
	static const type_info& material = typeid( CMaterial );
	...
	m_vecCloneResource[( UINT )eType].push_back( *ppRes );
}

template<typename T>
inline Ptr<T> CResourceManager::FindRes( const wstring & strKey )
{
	static const type_info& mesh = typeid( CMesh );
	static const type_info& shader = typeid( CShader );
	static const type_info& material = typeid( CMaterial );
	...
	iter = m_mapResource[( UINT )eType].find( strKey );

	if ( iter == m_mapResource[( UINT )eType].end() )
	{
		return nullptr;
	}
...
template<typename T>
inline Ptr<T> CResourceManager::Load( const wstring & strKey, const wstring & _strPath )
{
	static const type_info& mesh = typeid( CMesh );
	static const type_info& shader = typeid( CShader );
	static const type_info& material = typeid( CMaterial );
	...
	pRes = new T;
	wstring strFullPath = GET_SINGLE( CPathManager )->GetResPath();
