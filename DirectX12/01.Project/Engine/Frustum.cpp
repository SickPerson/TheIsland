...
CFrustum::CFrustum( CCamera * pCamera ) :
	m_pCamera(pCamera)
{
	Init();
}
...
{
	Matrix matViewInv = m_pCamera->GetViewMatInv();
	Matrix matProjInv = m_pCamera->GetProjMatInv();
...
bool CFrustum::CheckFrustum( const Vec3 & vPos )
{
	for ( UINT i = 0; i < ( UINT )FACE_TYPE::END; ++i )
	{
		Vec3 vNormal = m_arrFace[i];

		if ( vNormal.Dot( vPos ) + m_arrFace[i].w > 0 )
			return false;
	}
...
bool CFrustum::CheckFrustumSphere( const Vec3 & vPos, float fRadius )
{
	for ( UINT i = 0; i < ( UINT )FACE_TYPE::END; ++i )
	{
		Vec3 vNormal = m_arrFace[i];

		if ( vNormal.Dot( vPos ) + m_arrFace[i].w > fRadius )
			return false;
	}
